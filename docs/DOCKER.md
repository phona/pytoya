...
- **Web (Vite):** `src/apps/web/Dockerfile`
...
The web app reads `VITE_API_URL` at build time (bundled into the client). For an Ingress that routes `/api/*` to the backend, a good default is:

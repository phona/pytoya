...
import { ProjectSettingsExportScriptsPage } from './ProjectSettingsExportScriptsPage';
...
describe('ProjectSettingsExportScriptsPage', () => {
  beforeAll(() => {
  ...
  it('opens create dialog from empty state', async () => {
    ...
    await act(async () => {
      renderWithProviders(<ProjectSettingsExportScriptsPage />, { route: '/projects/1/settings/export-scripts' });
    });

    await screen.findByRole('heading', { name: /export scripts/i, level: 1 });

    const newButtons = screen.getAllByRole('button', { name: /new script/i });
    await user.click(newButtons[0]!);

    await screen.findByRole('heading', { name: /create export script/i });
  });
  ...
  it('confirms discard when closing dirty create dialog', async () => {
    ...
    expect(screen.queryByRole('heading', { name: /create export script/i })).not.toBeInTheDocument();
  });
});

...
export function ProjectForm({ project, onSubmit, onCancel, isLoading }: ProjectFormProps) {
  ...
  const [ocrModelId, setOcrModelId] = useState(project?.ocrModelId ?? '');
  const [llmModelId, setLlmModelId] = useState(project?.llmModelId ?? '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (project) {
      ...
      updateData.ocrModelId = ocrModelId || undefined;
      updateData.llmModelId = llmModelId || undefined;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label htmlFor="ocrModelId" className="block text-sm font-medium text-gray-700">
            OCR Model
          </label>
          <select
            id="ocrModelId"
